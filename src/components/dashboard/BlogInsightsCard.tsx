...
export const BlogInsightsCard = ({ metrics, topPosts = [] }: BlogInsightsCardProps) => {
        ...
        <Tabs defaultValue="overview" className="w-full">
          ...
          <TabsContent value="overview" className="space-y-4 mt-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <MetricBox icon={MessageSquare} label="Total Posts" value={metrics.totalBlogs} />
              <MetricBox icon={Eye} label="Total Views" value={formatNumber(metrics.totalViews)} />
              <MetricBox
                icon={Heart}
                label="Total Likes"
            ...
            <div className="pt-4 border-t">
              ...
              <p className="text-2xl font-bold mb-2">
                {metrics.totalBlogs > 0
                  ? (
                      ((metrics.totalLikes + metrics.totalComments) / (metrics.totalBlogs * 10)) *
                      100
                    ).toFixed(1)
                  : 0}
                %
              ...
              {chartData.map((post, index) => (
                <div key={index} className="space-y-1">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-muted-foreground">{formatNumber(post.value)}</span>
                  </div>
                  <div className="w-full bg-muted rounded-full h-2">
                    <div
